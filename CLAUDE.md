...
Archivo de Chile — a Laravel 12 SPA using Inertia.js v2 + Vue 3 + TypeScript. Authentication is handled by Laravel Fortify (headless) with full 2FA support. Local dev is served by Laravel Herd at `https://archivodechile.test`.
...
php artisan test --compact --filter=RegistrationTest  # Run specific test
php artisan test --compact tests/Feature/Auth/        # Run test directory

# Linting & Formatting
vendor/bin/pint --dirty --format agent    # Format modified PHP files (run after every PHP change)
npm run lint                              # ESLint fix
npm run format                            # Prettier fix
npm run types:check                       # Vue + TypeScript type checking

# Full CI check
composer run ci:check         # Runs lint:check, format:check, types:check, and tests
...
### Backend (Laravel 12)
- **Routing**: `bootstrap/app.php` configures middleware/exceptions. Routes in `routes/web.php` + `routes/settings.php`.
- **Controllers**: `app/Http/Controllers/Settings/` — Profile, Password, TwoFactorAuthentication.
- **Actions**: `app/Actions/Fortify/` — CreateNewUser, ResetUserPassword (Fortify pattern).
- **Concerns**: `app/Concerns/` — PasswordValidationRules, ProfileValidationRules (shared validation).
- **Middleware**: HandleInertiaRequests (shared props), HandleAppearance (theme persistence).
- **Database**: SQLite locally. Sessions, cache, and queues all use database driver.

### Frontend (Vue 3 + Inertia v2)
- **Pages**: `resources/js/pages/` — Vue SFCs mapped to Inertia routes (auth/, settings/, Welcome, Dashboard).
- **Components**: `resources/js/components/` — Reka UI-based component library (shadcn-style).
- **Layouts**: `resources/js/layouts/` — App and settings layout wrappers.
- **Composables**: `resources/js/composables/` — Vue 3 composables.
- **Wayfinder**: `resources/js/actions/` and `resources/js/routes/` — auto-generated TypeScript route functions. Import backend routes as `@/actions/...` or `@/routes/...`.
- **Styling**: Tailwind CSS v4 + tw-animate-css. Icons via lucide-vue-next.
